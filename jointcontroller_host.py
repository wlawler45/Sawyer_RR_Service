...
class Sawyer_impl(object):
    ...
    def command_cb(self,wire,value,time):
        ...
        if(wire==self.easy_position_command && self._mode==self.MODE_POSITION):
            joint_command=wire.InValue
            self._execute_joint_command(joint_command.command)
        if(wire==self.easy_velocity_command && self._mode==self.MODE_VELOCITY):
            velocity_command=wire.InValue
